...
# In this version, we take data from json for each new day.
# The program runs for one day, after which it updates the json file.

def get_population_data():
    """
    Get data about each person of the city with information for the current day
    :return:
    """
...
############# Work_places ###################
office_works = [Work(type_name="office") for _ in range(TOTAL_NUMBER_OF_WORKS//3)]
school_works = [Work(type_name="school") for _ in range(TOTAL_NUMBER_OF_WORKS//3)]
metro_works = [Work(type_name="metro") for _ in range(TOTAL_NUMBER_OF_WORKS//3)]
